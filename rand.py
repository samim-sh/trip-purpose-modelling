...
from tensorflow.keras.layers import LSTM, Dense, Embedding, Dropout, BatchNormalization, Masking
...
def create_df():
    ...
    print("window size, length, id", window_size, len(df_trip), len(df_trip.groupby('id')))
...
def preprocess_data(df):
    ...
    for _, group in grouped:
        ...
        X = group.tail(window_size).iloc[:-1].drop(columns=['id', 'WHYTRP1S', "R_RELAT", "age_category", 'R_SEX', 'WORKER', 'TRPTRANS', "TRAVDAY", "time_of_day"]).values
...
preds = np.argmax(model.predict(X_test[:100]), axis=1)
true = np.argmax(y_test[:100], axis=1)
